...
def gbId():
    ...
    Comp1=Comp.get()
    br1=br.get()
    cur.execute('''select * from Material where Id=?''',(Id1,))
    for record in cur:
        print('ID :',str(record[0])+'\n')
        print('Name:',str(record[1])+'\n')
        print('Date:',str(record[2])+'\n')
...
def Cus():
    ...
    Comp1=Comp.get()
    br1=br.get()
    cur.execute('''select * from Material where name=?''',(name1,))
    for record in cur:
        print('ID :',str(record[0])+'\n')
        print('Name:',str(record[1])+'\n')
        print('Date:',str(record[2])+'\n')
...
lb = tk.Label(Cus_frame,text="Material Vise Report",padx=10,pady=30,
...
Cus_ID=tk.Label(Cus_frame,text="material ID",padx=5,pady=30,font=('Bold',15)).grid(row=2,column=0)
Cus_id=tk.Entry(Cus_frame,textvariable=Id,font=('Bold',15)).grid(row=2,column=1)
    
Cus_Name=tk.Label(Cus_frame,text="Material",padx=5,pady=30,font=('Bold',15)).grid(row=2,column=2)
Cus_name=tk.Entry(Cus_frame,textvariable=name,font=('Bold',15)).grid(row=2,column=3,columnspan=2)

getBYid=tk.Button(Cus_frame,text="get Report By Material Id",command=gbId).grid(row=6,column=0)
getBYid=tk.Button(Cus_frame,text="get Report By Material Name",command=Cus).grid(row=6,column=1)
