...
cur= con.cursor()
cur.execute('''create table if not exists Destination(ID text,Name text,Date date,Time text)''')
def show_data():
    con= sqlite3.connect('WBMS.txt')
    cur= con.cursor()
...
def adde():           #ADDING DATA 
    Id1=Id.get()
    name1=name.get()
    cur.execute('''Insert into Destination(ID,Name,Date,Time)values(?,?,?,?)''',(Id1,name1,fx1,fx2))
    con.commit()
    show_data()

    tkinter.messagebox.showinfo('Congratulations !!','Destination successfully Added')

def upe():         #UPDATING DATA 
    Id1=Id.get()
    name1=name.get()
    cur.execute('''Update Destination SET Name=? WHERE ID=?''',(name1,Id1))
    con.commit()
    show_data()
    tkinter.messagebox.showinfo('Congratulations !!','Destination successfully updated')

def dele():
    Id1=Id.get()
    cur.execute('''Delete from Destination where ID=?''',(Id1,))
    con.commit()
    tkinter.messagebox.showinfo('Congratulations !!','Destination successfully Deleted')
    show_data()
...
lb = tk.Label(Cus_frame,text="UOM Details",padx=10,pady=30,
              font=('Bold',40)).grid(row=0,column=0,columnspan=6)
Cus_ID=tk.Label(Cus_frame,text="Destination ID",padx=5,pady=30,font=('Bold',15)).grid(row=1,column=0)
Cus_id=tk.Entry(Cus_frame,textvariable=Id,font=('Bold',15)).grid(row=1,column=1,columnspan=2)
    
Cus_Name=tk.Label(Cus_frame,text="Destination Name",padx=5,pady=30,font=('Bold',15)).grid(row=1,column=2)
